...
function ConvoStarterRoomCreate() {
  ...
  const showInstructions = () => {
    Swal.fire({
      ...
      html: `<div class="text-left grid grid-cols-[auto_1fr] gap-2">
        ${gamesData['convo-starter'].instructions.map((line, index) => `<span><b>Step ${index}</b></span></span>${line}</span>`).join('')}
      </div>
      `,
  ...
  return (
    <div className="p-2">
      ...
      <div className="text-left lg:text-center">
        ...
        <p className="max-w-3xl mb-4 ">{gamesData['convo-starter'].heroText} <span className="underline hover:text-ne-300 cursor-pointer" onClick={showInstructions}>How to Play.</span></p>
...
export default ConvoStarterRoomCreate;