...
export function SortableItem(props) {
  ...
  return (
    <div key={props.id} ref={setNodeRef} style={style} {...attributes} {...listeners} className='py-4 pe-4 rounded-md border-[1px] flex'>
      ...
      <div>
        ...
        {props.fields.map(fieldName => <p key={fieldName}>
          <span className={`font-bold ${fieldName === props.field ? 'animate-linear bg-[length:200%_auto] bg-gradient-to-r from-dt-500 to-dt-200 text-white p-1 rounded-md' : 'bg-gray-500/20  rounded-md p-1'}`}>{capitalise(fieldName)}</span> <span className="">{props.data[fieldName]}</span>
        </p>)}
        </ul>
      </div>
    </div>
