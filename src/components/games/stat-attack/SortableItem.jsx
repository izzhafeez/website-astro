...
export function SortableItem(props) {
  ...
  return (
    <div key={props.field} ref={setNodeRef} style={style} {...attributes} {...listeners} className='py-4 pe-4 rounded-md border-[1px] flex bg-dt-100/10'>
      ...
      <div>
        ...
        {props.fields.map(fieldName => <p>
          <span key={fieldName} className={`font-bold ${fieldName === props.field ? 'bg-dt-500 dark:bg-dt-300 text-white p-1 rounded-md' : 'bg-gray-500/20 dark:text-white rounded-md p-1'}`}>{capitalise(fieldName)}</span> <span className="dark:text-white">{props.data[fieldName]}</span>
        </p>)}
        </ul>
      </div>
    </div>
